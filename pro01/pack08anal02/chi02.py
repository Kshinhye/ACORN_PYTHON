...
# 가설검정은 수집된 데이터를 바탕으로한다.
# <적합도 검정실습>
# 주사위를 60 회 던져서 나온 관측도수 / 기대도수가 아래와 같이 나온 경우에 이 주사위는 적합한 주사위가 맞는가를 일원카이제곱 검정
# 으로 분석하자.
#
# 주사위 눈 금 1  2  3  4  5  6
# 관 측 도 수 4  6  17 16 8  9
# 기 대 도 수 10 10 10 10 10 10

#귀무가설을 엎어버리고 싶은거지
#귀무가설 : 기대치와 관측치(실제값)는 차이가 있다. 현재 주사위는 게임에 적합하다.      보수적
#대립가설 : 기대치와 관측치(실제값)는 차이가 있다. 현재 주사위는 게임에 적합하지 않다.   적극적
...
result=stats.chisquare(f_obs=data)  #이원제곱은 .chi2_contingency (파이썬은 두개를 구분한다.)
#print(result)
print('chi2: {}, p-value: {}'.format(result.statistic, result.pvalue))
print('statistic:%.5f, p-value:%.5f'%(result))
#statistic:14.20000, p-value:0.01439 이 둘은 반비례 관계다.
#
#판정 : p-value:0.01439 < 0.05 이므로 귀무가설 기각
#기대치와 관찰치는 차이가 있다. 현재 주사위는 게임에 적합하지 않다. 현재 관찰된 데이터는 우연히 발생한 데이터가 아니다.

#cv로 판정 df:? = (N-1) =5 
#cv:? 11.07 #statistic:14.20000  #귀무가설 기각 / 대립가설 채택
