'''
이원카이제곱
동질성 검정 - 두 집단의 분포가 동일한가? 다른 분포인가? 를 검증하는 방법이다. 두 집단 이상에서 각 범주(집단) 간의 비율이 서로
동일한가를 검정하게 된다. 두 개 이상의 범주형 자료가 동일한 분포를 갖는 모집단에서 추출된 것인지 검정하는 방법이다.
'''
import pandas as pd
import scipy.stats as stats

#===============================================================================
# 동질성 검정실습1) 교육방법에 따른 교육생들의 만족도 분석 - 동질성 검정 survey_method.csv
#===============================================================================

data=pd.read_csv("https://raw.githubusercontent.com/pykwon/python/master/testdata_utf8/survey_method.csv")
print(data.head(2))
print(data.method.unique()) #[1 2 3] 3개
print(data.survey.unique()) #[1 2 3 4 5] 5개

#대립: 교육방법에 따른 교육생들의 만족도에 차이가 있다.
#귀무: 교육방법에 따른 교육생들의 만족도에 차이가 없다.

ctab=pd.crosstab(index=data.method,columns=data.survey)
ctab.columns=["매우만족",'만족','보통','불만족','매우불만족']
ctab.index=['방법1','방법2','방법3']
print(ctab)

chi2,p,ddof,_=stats.chi2_contingency(ctab) #교차표의 결과를 직접 적용해도 됨
print('chi2: {}, p-value: {}, ddof: {}'.format(chi2,p,ddof))
#chi2: 6.544667820529891, p-value: 0.5864574374550608, ddof: 8
#해석 :  p-value: 0.5864574374550608 > 0.05 이므로 귀무채택

print('--------------------')
#===============================================================================
# 동질성 검정 실습2) 연령대별 sns 이용률의 동질성 검정
# 20대에서 40대까지 연령대별로 서로 조금씩 그 특성이 다른 SNS 서비스들에 대해 이용 현황을 조사한 자료를 바탕으로 연령대별로 홍보전략을 세우고자 한다.
# 연령대별로 이용 현황이 서로 동일한지 검정해 보도록 하자. 
#===============================================================================

# 귀무가설 : 연령대별로 sns 이용 현황이 서로 동일하다.
# 대립가설 : 연령대별로 sns 이용 현황이 서로 동일하지않다.

data2=pd.read_csv("../testdata/snsbyage.csv")
print(data2.head(3),len(data2))
print(data2['age'].unique()) #[1 2 3] 20대 30대 40대
print(data2['service'].unique()) #['F' 'T' 'K' 'C' 'E']
print()
ctab2=pd.crosstab(index=data2.age, columns=data2.service)
ctab2.index=['20대','30대','40대']
print(ctab2)

#자유도는 3-1 * 5-1  =8

chi2,p,ddof,_=stats.chi2_contingency(ctab2)
print('chi2: {}, p-value: {}, ddof: {}'.format(chi2,p,ddof))
#chi2: 102.75202494484225, p-value: 1.1679064204212775e-18, ddof: 8
#해석 : p-value: 1.1679064204212775e-18 < 0.05 이므로 귀무기각
#1439명을 대상으로 연령대별로 sns 이용 현황이 서로 동일한지 검정해보았다.
#그 결과 연령대별로 SNS 서비스 이용 현황은 서로 동일하지않다.

#===============================================================================
# 문제3) 카이제곱 검정 + 웹
# web(Django framework)에서 남여를 대상으로 선호하는 커피전문점을 선택하는 설문지 작성 후 커피전문점에 대한 소비자 인식조사를 한다. 
# 이 결과로 '성별에 따라 선호하는 커피브랜드에 차이가 있는지'를 검정하시오
#===============================================================================

