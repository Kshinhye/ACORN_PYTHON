# sklearn이 제공하는 Logistic Regression : 다항분류 (활성화함수로 softmax를 사용한다(씨그모어 안씀), 그래서 결과값을 확률로 반환한다.)
# 확률값이 가장 큰 녀석에 인덱스를 취한다.
# LogisticRegression은 범주형으로 다중 클래스(label)를 지원하도록 일반화되어있다. (softnax함수를 쓰기떄문)
# iris dataset을 사용해보자 (꽃의 종류를 세가지로 분류한다.)
# 이전에 배웠던 logit과 glm은 이항분류밖에 안된다.
...
# """
#
# #c=1.0 L2규제 패널티 적용(숫자값을 조정해가면서 정확도를 확인해가면된다.) 값이 작을수록 규제는 강화된다.
# model=LogisticRegression(C=1.0, random_state=0, verbose=0) #verbose 학습과정을 보여주는것 0:출력X, 1:자세히, 2:함축적 정보만
# print(model)
...
#
# #내가 굳이 보여주는 이유는 선형으로 분류됐다는거야 y=wx+b를 썻다는거야
# print((2*0.5*0.75)/(0.5+0.75))
# print(2*0.4*1/1.4)
# print(2*0.4*1/1.4)