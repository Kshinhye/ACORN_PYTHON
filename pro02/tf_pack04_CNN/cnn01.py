# 정의만 잘 내려주세요.
# Convoluted Neural Network(CNN)의 핵심은 이미지를 학습한다는 것
# 합성곱으로 이미지 특징 추출 후 완전 연결층(Dense)으로 전달하여 이미지를 분류함
...
# horizontal edge filter : 합성곱 필터(3*3) 적용-------- (수평)
filter1 = np.array([
    [ 1,  1,  1],
    [ 0,  0,  0],
    [-1, -1, -1]
...
# vertical edge filter : 합성곱 필터(3*3) 적용-------- (수직)
filter2 = np.array([
    [ -1,  0,  1],
    [ -1,  0,  1],
    [ -1,  0,  1]
...
new_image = np.zeros(im.shape)
im_pad = np.pad(im,1, 'constant') # stride 1로 줌 한칸식이동
