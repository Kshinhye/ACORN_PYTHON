# classfication: 주어진 feature에 대해 label로 학습시켜 데이터를 분류하는 방법
# 2항1열인데는 시그모이드를 사용한다. (정성적인 분류)
# h(x) =P(y=1|x;θ) x:feature, θ: model parameter
# hypothesis function 의 출력값은 주어진 feature x라는 값을 가질 때 class 1에 들어갈 확률 이라는 의미
...
meva2=model2.evaluate(x, y)
print(meval)

# 새로운 값으로 예측
pred = model2.predict(np.array([[1,2],[10,5]]))
print('예측 결과 : ', pred) 
